...
"""
    ...
    The frontend of this application uses a python interface to the Tk GUI toolkit known as tkinter.
    More information about tkinter can be found here: https://docs.python.org/3/library/tkinter.html

    To run our application, make sure you are currently in the /project-noodles. From here, run in a
    terminal `python3 application/ui.py`.

    This file does not implement the any core features itself; rather, it communicates with Bank.py to
    accomplish all of its functionality.

    The provided frontend is operating system independent; it will run on Windows, Linux, and Mac.
    However, the placement of the individual elements and the overall appearance will vary from
    system to system. 
    
    This version of ui.py is designed for Mac. If a given element is not visible or cannot be easily clicked, 
    try modifying the x and y coordinate locations of each individual element. Within each page class, 
    there will be a section that is denoted by the comment "#placement"; change the coordinate locations here.

    There are 7 pages in total: StartPage, InsidePage, AddPassword, ForgotPassword, AnswerSecurityQuestions, 
    SignUp, and AddSecurityQuestions. The structure of the general page navigation (less important links are omitted)
    is as follows:

    StartPage -+--> InsidePage --> AddPassword
               |
               +--> ForgotPassword --> AnswerSecurityQuestions
               |
               +--> SignUp --> AddSecurityQuestions
    
    Finally, all assets used in the frontend are located in project-noodles/application/assets/. Additional questions
    should be able to be resolved via the comments scattered within the file.
"""
...
_security_questions_1 = [
    '  Are you single? If so, why?', '  Why did you forget your password?',
    '  What is your favorite color?', '  What is your mother\'s maiden name?',
    '  Who is your favorite CS professor?'
]
_security_questions_2 = [
    '  What is your favorite TV program?',
    '  What team do you love to see lose?', '  Where did you meet your spouse?',
...
def _copy_clipboard(password):
    ...
    messagebox.showinfo("Password Copied",
                        "Current password successfully copied to clipboard.")
...
# Class for button that highlights on hover
...
# Class for a scrollable button list in a frame
...
class NoodlePasswordVault(tk.Tk):
    ...
    def create_inside(self):
        ...
        inside_frame = InsidePage(self.container, self,
                                  self.user_password_information)
    ...
    def create_security_q(self, username, password):
        ...
        security_frame = CreateSecurityQuestions(self.container, self,
    ...
    def create_security_aq(self, username):
        ...
        security_frame = AnswerSecurityQuestions(self.container, self,
                                                 self.username)
...
class StartPage(tk.Frame):
    ...
    def __init__(self, master, controller):
        ...
        # username entry
        username_entryline = tk.Label(self,
                                      image=entryline_final,
                                      background='#FFFFFF')
        ...
        self.pw_entry = tk.Entry(
            ...
            foreground='#757575',
            insertbackground='#757575',
            highlightthickness=0)  #show="*" changes input to *
        pw_text = tk.Label(self,
                           text="Password",
                           font=(TRUE_FONT, 10),
                           background='#FFFFFF',
                           foreground='#757575')
        ...
        self.error_text = tk.Label(
            self,
            text="The username or password you entered is incorrect or invalid.",
            font=(TRUE_FONT, 7),
            background='#FFFFFF',
            foreground='#9B1C31')

        # forgot password button
        forgot_pw_button = HoverButton(
            self,
            text="Forgot Password?",
            padx=-10,
            pady=-10,
            highlightthickness=0,
            command=lambda: _combine_funcs(
                controller.show_frame(ForgotPassword),
                self.exit_page(self.username_entry, self.pw_entry)),
            background='#FFFFFF',
            foreground='#757575',
            activebackground='#FFFFFF',
            activeforeground='#40c4ff',
            borderwidth=0)

        # log in button
        log_in_button_path = os.path.join(_assetdir, 'log_in.png')
        log_in_button_image = Image.open(log_in_button_path)
        log_in_button_resized = log_in_button_image.resize((250, 47),
                                                           Image.ANTIALIAS)
        log_in_button_final = ImageTk.PhotoImage(log_in_button_resized)
        log_in_button = tk.Button(
            self,
            image=log_in_button_final,
            padx=-10,
            pady=-5,
            borderwidth=0,
            background='#FFFFFF',
            command=lambda: self.query_login(controller, self.username_entry,
                                             self.pw_entry))
...
# Internal Page that displays Password List. Reach it from StartPage.
class InsidePage(tk.Frame):
    ...
    def __init__(self, master, controller, info, startPage=None):
        ...
        self.add_new_password_button = tk.Button(
            ...
            bg='#40c4ff',
            fg='#40c4ff',
            command=lambda: controller.show_frame(AddPassword))
        ...
        for i in range(len(self.lis)):
            btn = tk.Button(self.website_list.interior,
                            height=1,
                            width=20,
                            relief=tk.FLAT,
                            bg='#FFFFFF',
        ...
        # login information
        self.website_text = tk.Label(self,
                                     text="Website: " + self.website,
                                     font=(TRUE_FONT, 12),
                                     background='#FFFFFF',
                                     foreground='#757575')
        self.username_text = tk.Label(self,
                                      text="Username: " + self.username,
                                      font=(TRUE_FONT, 12),
                                      background='#FFFFFF',
                                      foreground='#757575')
        self.password_text = tk.Label(self,
                                      text="Password: " + self.password.get(),
                                      font=(TRUE_FONT, 12),
                                      background='#FFFFFF',
                                      foreground='#757575')
        ...
        delete_button_resized = delete_button_image.resize((143, 47),
                                                           Image.ANTIALIAS)
        delete_button_final = ImageTk.PhotoImage(delete_button_resized)
        self.delete_button = tk.Button(
            self,
            image=delete_button_final,
            padx=-20,
            pady=-10,
            borderwidth=0,
            background='#FFFFFF',
            command=lambda: _combine_funcs(self.remove_password(),
        ...
        password_button_resized = password_button_image.resize((143, 47),
                                                               Image.ANTIALIAS)
        password_button_final = ImageTk.PhotoImage(password_button_resized)
        self.password_button = tk.Button(self,
                                         image=password_button_final,
                                         padx=-20,
                                         pady=-10,
                                         borderwidth=0,
                                         background='#FFFFFF',
        ...
        log_out_button_resized = log_out_button_image.resize((143, 47),
                                                             Image.ANTIALIAS)
        log_out_button_final = ImageTk.PhotoImage(log_out_button_resized)
        self.log_out_button = tk.Button(self,
                                        image=log_out_button_final,
                                        padx=-20,
                                        pady=-10,
                                        borderwidth=0,
                                        background='#FFFFFF',
                                        command=lambda: _log_out(controller))
        self.log_out_button.image = log_out_button_final  # prevent garbage collection

        # copy clipboard button
        copy_clipboard_button_path = os.path.join(_assetdir,
                                                  'copy_clipboard.png')
        copy_clipboard_button_image = Image.open(copy_clipboard_button_path)
        copy_clipboard_button_resized = copy_clipboard_button_image.resize(
            (143, 47), Image.ANTIALIAS)
        copy_clipboard_button_final = ImageTk.PhotoImage(
            copy_clipboard_button_resized)
        self.copy_clipboard_button = tk.Button(
            self,
            image=copy_clipboard_button_final,
            padx=-20,
            pady=-10,
            borderwidth=0,
            background='#FFFFFF',
            command=lambda: _copy_clipboard(self.password.get()))
    ...
    def remove_password(self):
        ...
        if messagebox.askyesno("Confirmation",
                               "Do you really wish to delete this login?"):
...
# Page to add new Passwords. Reach it from InsidePage.
class AddPassword(tk.Frame):
    ...
    def __init__(self, master, controller):
        ...
        banner.create_text(200,
                           160,
                           fill='#FFFFFF',
                           font=(TRUE_FONT, 20, "bold"),
                           text="Trust in us. \nSecure your future.")
        banner.create_text(
            190,
            220,
        ...
        add_confirm_button_resized = add_confirm_button_image.resize(
            (143, 47), Image.ANTIALIAS)
        add_confirm_button_final = ImageTk.PhotoImage(
            add_confirm_button_resized)
        add_confirm_button = tk.Button(
            self,
            image=add_confirm_button_final,
            padx=-10,
            pady=-10,
            command=lambda: check_inputs(
                self, controller, self.website_entry.get(),
                self.username_entry.get(), self.pw_entry.get(),
                self.pw_confirm_entry.get()),
            background='#FFFFFF',
            borderwidth=0)
        ...
        # confirmation entry
        pw_confirm_entryline = tk.Label(self,
                                        image=entryline_final,
                                        background='#FFFFFF')
        pw_confirm_entryline.image = entryline_final
        self.pw_confirm_entry = tk.Entry(
            self,
            borderwidth=0,
            show="◕",
            width=26,
        ...
        # website entry
        website_entryline = tk.Label(self,
                                     image=entryline_final,
                                     background='#FFFFFF')
        ...
        # mismatch input
        self.mismatch_text = tk.Label(self,
        ...
        back_button = HoverButton(
            self,
            text="Go back",
            font=(TRUE_FONT, 8, "bold"),
            command=lambda: _combine_funcs(quit_page(self),
                                           controller.show_frame(InsidePage)),
            background='#FFFFFF',
            foreground='#757575',
            activebackground='#FFFFFF',
        ...
        def check_inputs(self, controller, website_entry, username_entry,
                         pw_entry, pw_confirm_entry):
...
# First Password Reset Page. Reach it from StartPage.
class ForgotPassword(tk.Frame):
    ...
    def __init__(self, master, controller):
        ...
        subtitle = tk.Label(
            self,
            text=
            "Don't worry, it happens to the best of us. Enter the username you are using\n\nbelow, relax, and we will get your account back in a jiffy.",
            font=(TRUE_FONT, 9),
            background='#FFFFFF',
            foreground='#757575')
        ...
        request_button = tk.Button(self,
                                   image=request_final,
                                   padx=-10,
                                   pady=-5,
                                   borderwidth=0,
                                   background='#FFFFFF',
                                   command=lambda: self.validate_username(
                                       controller, self.username_entry))
        ...
        back_button = HoverButton(
            ...
            activebackground='#FFFFFF',
            activeforeground='#40c4ff',
            command=lambda: _combine_funcs(self.exit(self.username_entry),
                                           controller.show_frame(StartPage)))

        # sign up button
        sign_up_button = HoverButton(
            self,
            text="Sign up.",
            borderwidth=0,
            command=lambda: _combine_funcs(self.exit(self.username_entry),
...
# Second Password Reset Page. Reach it from ForgotPassword.
class AnswerSecurityQuestions(tk.Frame):
    ...
    def __init__(self, master, controller, username):
        ...
        dropdown_1_text = tk.Label(self,
                                   text="Security Question 1",
                                   font=(TRUE_FONT, 8),
                                   background='#FFFFFF',
                                   foreground='#757575')
        var_1 = tk.StringVar()
        dropdown_1 = tk.OptionMenu(self,
                                   var_1,
                                   *_security_questions_1,
                                   command=self.get1)
        dropdown_1.config(width=24,
                          background='#FFFFFF',
                          activebackground='#FFFFFF',
                          anchor=tk.W,
                          borderwidth=1,
                          relief="ridge")
        dropdown_1["menu"].config(background='#FFFFFF',
                                  foreground='#757575',
                                  activebackground='#FFFFFF',
                                  activeforeground='#000000')

        # security question 1 response
        response_1_entryline = tk.Label(self,
                                        image=entryline_final,
                                        background='#FFFFFF')
        response_1_entryline.image = entryline_final
        self.response_1_entry = tk.Entry(self,
                                         borderwidth=0,
                                         highlightthickness=0,
                                         width=26,
                                         background='#FFFFFF',
        ...
        dropdown_2_text = tk.Label(self,
                                   text="Security Question 2",
                                   font=(TRUE_FONT, 8),
                                   background='#FFFFFF',
                                   foreground='#757575')
        var_2 = tk.StringVar()
        dropdown_2 = tk.OptionMenu(self,
                                   var_2,
                                   *_security_questions_2,
                                   command=self.get2)
        dropdown_2.config(width=24,
                          background='#FFFFFF',
                          activebackground='#FFFFFF',
                          anchor=tk.W,
                          borderwidth=1,
                          relief="ridge")
        dropdown_2["menu"].config(background='#FFFFFF',
                                  foreground='#757575',
                                  activebackground='#FFFFFF',
                                  activeforeground='#000000')

        # security question 2 response
        response_2_entryline = tk.Label(self,
                                        image=entryline_final,
                                        background='#FFFFFF')
        response_2_entryline.image = entryline_final
        self.response_2_entry = tk.Entry(self,
                                         borderwidth=0,
                                         highlightthickness=0,
                                         width=26,
                                         background='#FFFFFF',
        ...
        # invalid input
        self.invalid_text = tk.Label(
            self,
            text="Your answers do not match our records.",
            font=(TRUE_FONT, 7),
            background='#FFFFFF',
            foreground='#9B1C31')
        ...
        confirm_button_resized = confirm_button_image.resize((143, 47),
                                                             Image.ANTIALIAS)
        confirm_button_final = ImageTk.PhotoImage(confirm_button_resized)
        confirm_button = tk.Button(
            self,
            image=confirm_button_final,
            padx=-100,
            pady=-100,
            command=lambda: self.on_confirm_press(controller),
            background='#FFFFFF',
            borderwidth=0)
        ...
        cancel_button_resized = cancel_button_image.resize((143, 47),
                                                           Image.ANTIALIAS)
        cancel_button_final = ImageTk.PhotoImage(cancel_button_resized)
        cancel_button = tk.Button(
            self,
            image=cancel_button_final,
            padx=-100,
            pady=-100,
            command=lambda: self.clear_entries(controller),
            background='#FFFFFF',
            borderwidth=0)
    ...
    def on_confirm_press(self, controller):
        if self.response_1_entry.get() == '' or self.response_2_entry.get(
        ) == '' or self.pw_entry.get() == '':
            self.mismatch_text.config(foreground='#FFFFFF')
            self.mismatch_text.lower()
            ...
            if bank.forgot_password(self.username, self.pw_entry.get(),
                                    self.response_1_entry.get(),
                                    self.response_2_entry.get()):
...
class SignUp(tk.Frame):
    ...
    def __init__(self, master, controller):
        ...
        sign_up_button = tk.Button(self,
                                   ...
                                   command=lambda: self.check_inputs(
                                       controller, self.username_entry, self.
                                       pw_entry, self.pw_confirm_entry),
                                   background='#FFFFFF',
                                   borderwidth=0)
        sign_up_button.image = sign_up_button_final  # prevent garbage collection
        ...
        # existing username
        self.username_error_text = tk.Label(
            self,
        ...
        back_button = HoverButton(
            ...
            text="Log in.",
            font=(TRUE_FONT, 8, "bold"),
            command=lambda: _combine_funcs(controller.show_frame(StartPage),
                                           self.quit_page()),
            background='#FFFFFF',
            foreground='#757575',
            activebackground='#FFFFFF',
    ...
    # Utility functions
    def check_inputs(self, controller, username_entry, pw_entry,
                     pw_confirm_entry):
...
# Second Page of Create Account. Reach it from SignUp.
class CreateSecurityQuestions(tk.Frame):
    ...
    def __init__(self, master, controller, username, password):
        ...
        # title
        title = tk.Label(self,
                         text="Set Your Security Questions",
                         font=(TRUE_FONT, 20),
                         background='#FFFFFF',
                         foreground='#000000')
        subtitle = tk.Label(
            self,
            text=
            "Please choose two security questions and answer them.\nWe will use them to help you recover your account\nin the event you ever lose it.",
            font=(TRUE_FONT, 8),
            background='#FFFFFF',
            foreground='#757575')
        ...
        confirm_button_image = Image.open(confirm_button_path)
        confirm_button_resized = confirm_button_image.resize((250, 47),
                                                             Image.ANTIALIAS)
        confirm_button_final = ImageTk.PhotoImage(confirm_button_resized)
        confirm_button = tk.Button(
            self,
            ...
            padx=-100,
            pady=-100,
            command=lambda: self.validate_inputs(
                controller, self.response_1_entry, self.response_2_entry),
            background='#FFFFFF',
            borderwidth=0)
        confirm_button.image = confirm_button_final  # prevent garbage collection

        # return to sign in button
        back_button = HoverButton(self,
                                  ...
                                  activebackground='#FFFFFF',
                                  activeforeground='#40c4ff',
                                  command=lambda: self.quit(controller))
    ...
    def validate_inputs(self, controller, response_1, response_2):
        ...
        if string_1 == "" or string_2 == "":  #or self.resp1 == "" or self.resp2 == ""
            ...
            bank.sign_up(self.username, self.password,
                         (self.resp1, self.response_1_entry.get()),
                         (self.resp2, self.response_2_entry.get()))
            print(self.response_1_entry.get())
            print(self.response_2_entry.get())
            print(self.username)
            self.error_text.config(foreground='#FFFFFF')
...
if __name__ == "__main__":
    ...
    if platform.system() == 'Windows':
        ...
        iconfiles = [
            os.path.join(_assetdir, 'black_noodles_white_Xbg_icon%s' % (ext))
        ]
        icons = [
            tk.PhotoImage(master=application_process, file=iconfile)
