...
async def execute_read(n_bytes):
    loop = asyncio.get_running_loop()
    return await loop.run_in_executor(None, sys.stdin.buffer.read, n_bytes)


async def execute_write(msg):
    loop = asyncio.get_running_loop()
...
class ExtensionClient:
    ...
    async def send_message_bank(self):
        while True:
            ...
            print("Sending message bank", file=sys.stderr)
